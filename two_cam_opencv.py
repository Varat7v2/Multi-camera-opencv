...
def main():
    ...
    output_vid = cv2.VideoWriter('./output/two_camera_test.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (lwidth*2,lheight))
    ...
    while(True): 
        ...
        for score, box in zip(lscores, lboxes):
            if score > 0.5:
                ...
                landmark_offset = -0.0003666*area+4.4425
                ...
                for i in range(67):
                    ...
                    cv2.circle(rframe, (int(transformed_point[0][0]-linear_offset_x-landmark_offset), 
                        int(transformed_point[1][0]+20-landmark_offset)), 1, (0,0,255), -1)
                    if i == 60:
                        diff = rshape.part(i).x - (transformed_point[0][0]-linear_offset_x)
                        if (-20<=diff<=20):
                            landmark_x_offset.append(diff)
                        else:
                            landmark_x_offset.append('NA')

        window = np.hstack([lframe, rframe])
        cv2.imshow('Two camera setup', window)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'): 
            df = pd.DataFrame(list(zip(box_area, box_dist_x, box_dist_y, rleft_correction, rright_correction, landmark_x_offset)), 
                columns =['box_area', 'box_dist_x', 'box_dist_y', 'rleft_correction', 'rright_correction', 'landmark_x_offset'])
            df.to_csv('data/two_camera_data.csv') 
            break
